...
const ModalImages = ({
  ...
  useEffect(() => {
    ...
    setLoading(true)
    getFiles(`images/${currentUser.uid}`, (url) => {
      setStore(url)
      setLoading(false)
    })
              ...
              <div className='px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4'>
                <div className='sm:flex sm:items-start'>
                  <div className='w-full mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left'>
                    <Dialog.Title
                      as='h3'
                      className='flex-1 text-lg font-medium leading-6 text-center text-gray-900 cursor-pointer'
                    >
                      My Storage
                    </Dialog.Title>
                      ...
                      >
                        ...
                        {store.map((item) => (
                          <img
                            src={item}
                            alt='img'
                            className={`object-cover w-full h-24 p-1 border rounded-sm cursor-pointer hover:scale-110 ${isActive(item)}`}
